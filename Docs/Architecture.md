...
| B19 | Party UI is built in code from `FormationData` (one view per slot); the scene only provides the host node, so a bigger board is a data change, never a scene rebuild | 10c | Locked |

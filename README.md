...
| Key | Action |
|---|---|
| `Tab` | Cycle pages (Battle / Party / Upgrades / Ascend / Shop) |
...
1. **Combat / pacing** — Play, watch the log feed, confirm stage 1 clears in ~2 minutes and the boss
   wave pays gems.
2. **Formation** — on the **PARTY** tab, tap a hero and then tap a slot to move it (occupied slots swap, empty slots
   receive, the old slot is vacated). The back rank is *attacked less often* (~1 swing in 4 with the shipped
   weighting) but never takes reduced damage. The battle screen shows the same board, display only.
